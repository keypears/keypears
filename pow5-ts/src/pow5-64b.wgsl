// pow5-64b constants
// 64 bytes = 32-byte nonce + 32-byte challenge
const HEADER_SIZE: u32 = 64;
const NONCE_START: u32 = 0;
const NONCE_END: u32 = 32;
...
// The matmul-based ASIC-resistant computation
// Same algorithm as 217a, just with 64-byte input
fn matmul_work(header: array<u32, HEADER_SIZE>) -> array<u32, HASH_SIZE> {
    ...
    var result: array<u32, HASH_SIZE> = blake3_hash_128(&final_pre_hash);

    return result;
}

// debug: get matmul work result
@compute @workgroup_size(1, 1, 1)
fn debug_matmul_work(@builtin(global_invocation_id) global_id: vec3<u32>) {
    var work: array<u32, HASH_SIZE> = matmul_work(header);

    var compressed_work: array<u32, COMPRESSED_HASH_SIZE>;
    for (var i: u32 = 0; i < COMPRESSED_HASH_SIZE; i++) {
        compressed_work[i] = 0;
    }
    for (var i: u32 = 0; i < HASH_SIZE; i++) {
        var j: u32 = i / 4;
        var k: u32 = 3 - (i % 4);
        compressed_work[j] += (work[i] & 0xff) << (k * 8);
    }

    for (var i: u32 = 0; i < COMPRESSED_HASH_SIZE; i++) {
        final_result.hash[i] = compressed_work[i];
    }
}

// Elementary iteration for pow5-64b
// Unlike 217a, we don't insert work_par into the header.
// We simply hash the matmul result to produce the final PoW hash.
fn elementary_iteration(header: array<u32, HEADER_SIZE>) -> array<u32, COMPRESSED_HASH_SIZE> {
    // Compute the matmul work (same ASIC-resistant computation)
    var work: array<u32, HASH_SIZE> = matmul_work(header);

    // Final hash: double-hash the matmul result
    var hash_1: array<u32, 32> = blake3_hash_32(&work);
    var hash_2: array<u32, 32> = blake3_hash_32(&hash_1);

    // Compress the hash for output
...
fn workgroup_reduce(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    // For 64b, the nonce is 32 bytes (bytes 0-31).
    // We use global_thread_id to set the LAST 4 bytes of the nonce (bytes 28-31).
    // This gives us 2^32 iterations per GPU dispatch while keeping the first
    // 28 bytes of the nonce for higher-order iteration on the CPU side.
    var nonce: u32 = global_thread_id;

    // Update the last 4 bytes of the 32-byte nonce in big-endian order
    // Nonce bytes 28-31 (which is u32 indices 28, 29, 30, 31)
    local_header[28] = (nonce >> 24) & 0xff;
    local_header[29] = (nonce >> 16) & 0xff;
    local_header[30] = (nonce >> 8) & 0xff;
    local_header[31] = nonce & 0xff;

    // Run the elementary iteration
    ...
    // Check if the result_hash is less than the target
    ...
    // If the result is lower, store it
    if is_result_lower {
