...
name: claude-review
description:
  "Ask Claude to review KeyPears work. Use when the user requests Claude review
  or during large/risky experiments before acting on conclusions."
---

# Claude Review

Use Claude as an independent external reviewer for KeyPears issue designs, code
diffs, experiment results, and architectural decisions. In the KeyPears review
mode taxonomy this skill implements `external-claude`.
...
- an experiment is large, risky, or touches cryptography, authentication,
  federation, proof-of-work, or persistent state;
- an experiment has already failed once and the next design needs review;
- a code change affects architecture, protocol boundaries, public API behavior,
  client-side key handling, federation, or persistent state;
...
1. **Prepare the review context.** Include only the context Claude needs. Prefer
   ...
   - relevant dependency, protocol, or schema diff;
...
claude -p \
  --output-format json \
  --session-id <uuid> \
  --add-dir /Users/ryan/dev/keypears \
...
```text
You are reviewing KeyPears work. Take a code-review stance: findings first,
