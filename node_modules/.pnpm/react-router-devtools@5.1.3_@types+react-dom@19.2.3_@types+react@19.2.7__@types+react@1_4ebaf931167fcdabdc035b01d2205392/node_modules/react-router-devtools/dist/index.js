...
var DEFAULT_EDITOR_CONFIG = {
  ...
  open: async (path, lineNumber, columnNumber) => {
    ...
    exec(
      `code -g "${normalizePath2(path).replaceAll("$", "\\$")}${lineNumber ? `:${lineNumber}` : ""}${columnNumber ? `:${columnNumber}` : ""}"`
    );
  }
};
...
var handleWriteFile = async ({ path, options, openInEditor, appDir }) => {
  ...
  openInEditor(outputFile, void 0, void 0);
};
...
var getPropsNameFromFunctionDeclaration = (functionDeclaration) => {
  let propsName = null;
...
var transformJSX = (element, propsName, file) => {
  ...
  const hasDataSource = element.node.attributes.some(
    (attr) => attr.type === "JSXAttribute" && attr.name.type === "JSXIdentifier" && attr.name.name === "data-rrdt-source"
  );
...
function addSourceToJsx(code, id) {
  const [filePath] = id.split("?");
  const location = filePath?.replace(normalizePath3(process.cwd()), "");
  try {
    const ast = parse(code, {
      sourceType: "module",
      plugins: ["jsx", "typescript"]
    });
    const didTransform = transform4(ast, location);
    if (!didTransform) {
      return { code };
    }
    return gen(ast, {
...
var reactRouterDevTools = (args) => {
  ...
  const isTransformable = (id) => {
    ...
    const routeId = id.replace(normalizePath4(process.cwd()), "").replace(`/${appDirName}/`, "").replace(".tsx", "").replace(".ts", "");
    return routeId;
  };
  ...
  return [
    ...
    },
    {
      ...
      async configureServer(server2) {
        ...
        server2.middlewares.use(
          (req, res, next) => handleDevToolsViteRequest(req, res, next, (parsedData) => {
            ...
            if (routine === "request-event") {
              ...
              server2.hot.send("request-event", JSON.stringify(parsedData));
              return;
            }
            const id = data.id;
            ...
            server2.hot.send("route-info", JSON.stringify({ type, data }));
          })
        );
        server2.hot.on("all-route-info", (data, client) => {
          client.send(
            "all-route-info",
            JSON.stringify({
              type: "all-route-info",
        ...
        server2.hot.on("routes-info", (data, client) => {
          client.send(
            "routes-info",
            JSON.stringify({
              type: "routes-info",
        ...
        if (!server2.config.isProduction) {
          ...
          server2.hot.on(
            "open-source",
            (data) => handleOpenSource({
              data: {
                ...data,
                data: {
                  ...data.data,
                  source: data.data.source ? normalizePath4(`${process.cwd()}/${data.data.source}`) : void 0
                }
              },
              openInEditor,
              appDir
          ...
          server2.hot.on("add-route", (data) => handleWriteFile({ ...data, openInEditor, appDir }));
        }
      }
    },
    {
      ...
      async transform(code, id) {
        ...
        if (code.includes("console.")) {
          ...
          return lines.map((line, lineNumber) => {
            ...
            const location = `${id.replace(normalizePath4(process.cwd()), "")}:${lineNumber + 1}:${column + 1}`;
            const logMessage = `'${chalk.magenta("LOG")} ${chalk.blueBright(`${location} - http://localhost:${port}/_rrdt/open-source?source=${encodeURIComponent(id.replace(normalizePath4(process.cwd()), ""))}&line=${lineNumber + 1}&column=${column + 1}`)}\\n \u2192 '`;
