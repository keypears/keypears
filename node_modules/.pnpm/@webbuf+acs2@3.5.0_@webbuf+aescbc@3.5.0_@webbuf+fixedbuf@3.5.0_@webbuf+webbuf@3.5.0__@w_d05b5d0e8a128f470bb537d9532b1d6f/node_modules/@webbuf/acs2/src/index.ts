...
/**
 * Encrypt data with AES + CBC mode and a SHA-256 HMAC. Good for small amounts of
 ...
 */
export function acs2Encrypt(
  plaintext: WebBuf,
  aesKey: FixedBuf<32>,
  iv?: FixedBuf<16>,
) {
  const ciphertext = aescbcEncrypt(plaintext, aesKey, iv);
  const hmacbuf = sha256Hmac(aesKey.buf, ciphertext);
  return WebBuf.concat([hmacbuf.buf, ciphertext]);
}

/**
 * Decrypt data with AES + CBC mode and a SHA-256 HMAC.
 *
 * ACS2 = AES + CBC + SHA256 Hmac
 *
 ...
 */
export function acs2Decrypt(ciphertext: WebBuf, aesKey: FixedBuf<32>) {
  ...
  const hmacbuf2 = sha256Hmac(aesKey.buf, ciphertext);
