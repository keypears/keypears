...
describe("Pow5_217a tests", async () => {
  ...
  test("debug: hash header 128", async () => {
...

    {
      ...
      // now, generate the same hash with blake3Hash
      const hashHex = blake3Hash(headerAllOnes.buf.slice(0, 128)).toHex();
      expect(result.hash.toHex()).toBe(hashHex);
    }
  });
  ...
  test("debug: hash header 32", async () => {
    {
      ...
      // now, generate the same hash with blake3Hash
      const hashHex = blake3Hash(headerAllZeroes.buf.slice(0, 32)).toHex();
      expect(result.hash.toHex()).toBe(hashHex);
    }

    {
      ...
      // now, generate the same hash with blake3Hash
      const hashHex = blake3Hash(headerAllOnes.buf.slice(0, 32)).toHex();
      expect(result.hash.toHex()).toBe(hashHex);
    }
  });
