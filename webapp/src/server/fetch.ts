...
function ipv6ToBigInt(address: string): bigint | null {
  ...
  if (ipv4Match) {
    ...
    normalized = normalized.slice(0, -ipv4Match[1].length) + ipv4Groups.join(":");
  }
...
export async function resolveFederationAuthority(
  ...
  options: Pick<SafeFederationFetchOptions, "lookup" | "nodeEnv"> = {},
): Promise<LookupAddress[]> {
  ...
  const allowDevTest =
    options.nodeEnv !== "production" && isDevTestAuthority(authority);
  if (blocked.length > 0 && !allowDevTest) {
