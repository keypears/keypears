...
- Server stores SHA-256 hash of token (not raw token)
...
- Session tokens generated with `FixedBuf.fromRandom(32)` (cryptographically
  secure)
- Server stores only SHA-256 hash of tokens (64-char hex)
...
- Server storage: SHA-256 hash only (same pattern as login key hashing)
