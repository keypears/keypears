...
- [ ] Replace `"@webbuf/acb3": "^3.3.0"` with `"@webbuf/acs2": "^3.3.0"`
- [ ] Replace `"@webbuf/blake3": "^3.3.0"` with `"@webbuf/sha256": "^3.3.0"`
...
- [ ] Run `pnpm install` from root to update lock file
...
- [ ] Change `import { acb3Decrypt, acb3Encrypt } from "@webbuf/acb3"` to
      `import { acs2Decrypt, acs2Encrypt } from "@webbuf/acs2"`
- [ ] Change `import { blake3Hash, blake3Mac } from "@webbuf/blake3"` to
      `import { sha256Hash, sha256Hmac } from "@webbuf/sha256"`

### Update exports

- [ ] Change `export { blake3Hash, blake3Mac, acb3Encrypt, acb3Decrypt, ... }`
      to `export { sha256Hash, sha256Hmac, acs2Encrypt, acs2Decrypt, ... }`

### Rename blake3Pbkdf function

- [ ] Rename function `blake3Pbkdf` to `sha256Pbkdf`
- [ ] Update JSDoc comment to reference SHA-256 instead of Blake3
- [ ] Update internal calls from `blake3Mac` to `sha256Hmac`

### Update salt derivation functions

- [ ] `derivePasswordSalt`: Replace `blake3Hash` with `sha256Hash`, `blake3Mac`
      with `sha256Hmac`
- [ ] `deriveEncryptionSalt`: Replace `blake3Hash` with `sha256Hash`
- [ ] `deriveLoginSalt`: Replace `blake3Hash` with `sha256Hash`
- [ ] `deriveServerHashedLoginKeySalt`: Replace `blake3Hash` with `sha256Hash`

### Update key derivation functions

- [ ] `derivePasswordKey`: Replace `blake3Hash` with `sha256Hash`, `blake3Mac`
      with `sha256Hmac`, `blake3Pbkdf` with `sha256Pbkdf`
- [ ] `deriveEncryptionKey`: Replace `blake3Pbkdf` with `sha256Pbkdf`
- [ ] `deriveLoginKey`: Replace `blake3Pbkdf` with `sha256Pbkdf`
- [ ] `deriveHashedLoginKey`: Replace `blake3Hash` with `sha256Hash`,
      `blake3Mac` with `sha256Hmac`, `blake3Pbkdf` with `sha256Pbkdf`

### Update encryption functions

- [ ] `hashKey`: Replace `blake3Hash` with `sha256Hash`
- [ ] `encryptKey`: Replace `acb3Encrypt` with `acs2Encrypt`
- [ ] `decryptKey`: Replace `acb3Decrypt` with `acs2Decrypt`
- [ ] `encryptPassword`: Replace `acb3Encrypt` with `acs2Encrypt`
- [ ] `decryptPassword`: Replace `acb3Decrypt` with `acs2Decrypt`

### Update JSDoc comments

- [ ] Update all JSDoc comments that reference "Blake3" to reference "SHA-256"
- [ ] Update comment for `sha256Pbkdf` to explain it uses SHA-256 HMAC
...
- [ ] Update any comments referencing Blake3 to reference SHA-256
