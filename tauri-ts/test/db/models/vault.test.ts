...
describe("Vault Model", () => {
  ...
  describe("createVault", () => {
    ...
    it("should enforce unique name+domain combination", async () => {
      await createVault("uniquevault", "keypears.localhost", "0".repeat(64), "1".repeat(64));

      await expect(
        createVault("uniquevault", "keypears.localhost", "0".repeat(64), "1".repeat(64)),
      ).rejects.toThrow();
    });

    it("should allow same name on different domains", async () => {
      await createVault("samename", "keypears.localhost", "0".repeat(64), "1".repeat(64));
      const vault2 = await createVault("samename", "hevybags.localhost", "0".repeat(64), "1".repeat(64));
  ...
  describe("getVault", () => {
    it("should retrieve a vault by ID", async () => {
      const created = await createVault("findme", "keypears.localhost", "0".repeat(64), "1".repeat(64));
  ...
  describe("getVaultByNameAndDomain", () => {
    it("should retrieve a vault by name and domain", async () => {
      const created = await createVault("myvault", "keypears.localhost", "0".repeat(64), "1".repeat(64));
    ...
    it("should distinguish between domains", async () => {
      const vault1 = await createVault("samename", "keypears.localhost", "0".repeat(64), "1".repeat(64));
      await createVault("samename", "hevybags.localhost", "0".repeat(64), "1".repeat(64));
  ...
  describe("getVaults", () => {
    it("should return all vaults", async () => {
      await createVault("vault1", "keypears.localhost", "0".repeat(64), "1".repeat(64));
      await createVault("vault2", "keypears.localhost", "0".repeat(64), "1".repeat(64));
      await createVault("vault3", "keypears.localhost", "0".repeat(64), "1".repeat(64));
  ...
  describe("countVaults", () => {
    it("should return correct count", async () => {
      await createVault("vault1", "0".repeat(64), "0".repeat(64), "1".repeat(64));
      await createVault("vault2", "0".repeat(64), "0".repeat(64), "1".repeat(64));
  ...
  describe("deleteVault", () => {
    ...
    it("should decrease vault count after deletion", async () => {
      await createVault("vault1", "keypears.localhost", "0".repeat(64), "1".repeat(64));
      const vault2 = await createVault("vault2", "keypears.localhost", "0".repeat(64), "1".repeat(64));
